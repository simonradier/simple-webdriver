    ...
    afterEach(async function () {
      if (!nock.isActive()) {
        if (browserType === 'Safari' || browserType === 'Firefox')
          // wait 1 sec for Safari or Firefox to avoid "Could not create a session error"
    ...
    describe('start', function () {
      ...
      it('should throw an exception if the server is not a webdriver server 1/6 | Nock Only', async function () {
        ...
        nock(td.WD_SERVER_URL_HTTP[browserType])
          .post('/session')
          .reply(resp.code, resp.body, { 'Content-Type': 'test' })
        let driver: WebDriver
        driver = new WebDriver(td.WD_SERVER_URL_HTTP[browserType])
        await expect(
          driver.start(BrowserType[browserType], Capabilities.default)
      ...
      it('should start a session if webdriver response is correct (https) | Nock Only', async function () {
        let resp = td.WD_START_SESSION_RESPONSE.OK
        //@ts-ignore
        nock(td.WD_SERVER_URL_HTTPS[browserType])
          .post('/session')
          .reply(resp.code, resp.body, resp.headers)
        let resp2 = td.WD_WINDOW_HANDLE_RESPONSE.OK
        //@ts-ignore
